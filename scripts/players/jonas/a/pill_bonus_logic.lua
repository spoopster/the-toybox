...
local function addPillBonus(_, pillEffect, player, flags, pillColor)
    ...
    data.PILLS_POPPED = (data.PILLS_POPPED or 0)+1
    data.RESET_BOOST_ROOMS = 0

    if(data.PILLS_POPPED>=(data.PILLS_FOR_NEXT_BONUS or data.PILLS_FOR_BONUS or data.PILLS_FOR_BONUS_BASE or 3)) then
        data.PILLS_FOR_BONUS = (((data.PILL_BONUS_COUNT or 0)>=(data.PILLBONUS_EXTRA_REQ or 3)) and data.PILLBONUS_INCREMENT_EXTRA or data.PILLBONUS_INCREMENT or 3)
...
local function evalBonus(_, player, flag)
    ...
    elseif(flag==CacheFlag.CACHE_FIREDELAY) then
        player.MaxFireDelay = mod:addTps(player, (data.PILLBONUS_TEARS or 0.2)*mult)
