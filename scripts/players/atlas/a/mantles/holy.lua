...
local function useMantle(_, _, player, _)
    if(player:GetPlayerType()==mod.PLAYER_ATLAS_A) then
        mod:giveMantle(player, mod.MANTLES.HOLY)
    else
...
local function evalCache(_, player, flag)
    ...
    local numMantles = mod:getNumMantlesByType(player, mod.MANTLES.HOLY)

    if(flag==CacheFlag.CACHE_RANGE) then
        player.TearRange = player.TearRange+ENUM_RANGE_BONUS*numMantles*40
    end
    if(mod:atlasHasTransformation(player, mod.MANTLES.HOLY)) then
...
function(_, e)
    ...
    local chance = ENUM_SACRED_CHANCE*mod:getNumMantlesByType(p, mod.MANTLES.HOLY)
