...
HP_SPRITE:Render(mod:getHeartHudPosition())
...
local function postRender(_)
    ...
    Isaac.RenderText("CURRENT TRANSF", xPos, yPos+17, 1,1,1,1)
    Isaac.RenderText(mod:getMantleNameFromType(data.TRANSFORMATION), xPos, yPos+30, 1,1,1,1)

    Isaac.RenderText("OLD TRANSF", xPos, yPos+47, 1,1,1,1)
    Isaac.RenderText(mod:getMantleNameFromType(data.BIRTHRIGHT_TRANSFORMATION), xPos, yPos+60, 1,1,1,1)

    Isaac.RenderText(tostring(data.SALT_AUTOTARGET_ENABLED), xPos, yPos+77, 1,1,1,1)
end
