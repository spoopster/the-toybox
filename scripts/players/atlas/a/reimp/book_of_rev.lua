...
function(_, _, rng, player, flags, slot)
    ...
    mod:setMantleType(player, data.HP_CAP, nil, mod.MANTLE_DATA.METAL.ID)

    mod:spawnShardsForMantle(player, mod:getRightmostMantleIdx(player), 5)

    if(flags & UseFlag.USE_NOANIM == 0) then
        player:AnimateCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_REVELATIONS, "UseItem")
    end
    if(flags & UseFlag.USE_VOID == 0) then
        ItemOverlay.Show(Giantbook.BOOK_OF_REVELATIONS, nil, player)
    end
