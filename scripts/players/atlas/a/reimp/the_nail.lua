...
function(_, _, rng, player, flags, slot)
    ...
    mod:setMantleType(player, data.HP_CAP, nil, mod.MANTLE_DATA.DARK.ID)

    mod:spawnShardsForMantle(player, mod:getRightmostMantleIdx(player), 5)

    player:GetEffects():AddCollectibleEffect(CollectibleType.COLLECTIBLE_THE_NAIL)
    if(flags & UseFlag.USE_NOANIM == 0) then
        player:AnimateCollectible(CollectibleType.COLLECTIBLE_THE_NAIL, "UseItem")
    end
