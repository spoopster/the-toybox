...
local function evalCache(_, player, flag)
    if(not player:HasCollectible(mod.COLLECTIBLE.GOAT_MILK)) then return end
    local mult = player:GetCollectibleNum(mod.COLLECTIBLE.GOAT_MILK)
...
local function playerAttack(_, ent, weap, dir, cMod)
    ...
    if(p and p:HasCollectible(mod.COLLECTIBLE.GOAT_MILK) and weap:GetFireDelay()>0) then
        weap:SetFireDelay(skewWeaponFiredelay(weap:GetFireDelay(), p:GetCollectibleRNG(mod.COLLECTIBLE.GOAT_MILK), p:GetCollectibleNum(mod.COLLECTIBLE.GOAT_MILK)))
    end
end
