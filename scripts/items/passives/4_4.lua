...
local function fireSonicWaves(_, player)
    if(not player:HasCollectible(mod.COLLECTIBLE_4_4)) then return end
...
local function playerUpdate(_, player)
    local data = mod:getEntityDataTable(player)
    if(player:HasCollectible(mod.COLLECTIBLE_4_4)) then
    ...
    if(data.FOURFOUR_FIRE_DATA) then
        ...
        if(data.FOURFOUR_FIRE_DATA.DURATION%(SONIC_WAVE_FREQ*2)==0) then
            ...
            for i=1, tearsNum do
                ...
                local tear = Isaac.Spawn(2,mod.TEAR_SOUNDWAVE,0,player.Position,Vector.FromAngle(angle)*SONIC_WAVE_SPEED, player):ToTear()
...
local function tryInflictOverflow(_, ent, amount, flags, source, frames)
    ...
    if(not PlayerManager.AnyoneHasCollectible(mod.COLLECTIBLE_4_4)) then return end

    local luckMult = 0
    for i=0, Game():GetNumPlayers()-1 do
        local p = Isaac.GetPlayer(i)
        luckMult = luckMult+p:GetCollectibleNum(mod.COLLECTIBLE_4_4)*p.Luck
    end
    ...
    if(chance and Isaac.GetPlayer():GetCollectibleRNG(mod.COLLECTIBLE_4_4):RandomFloat()<chance) then
