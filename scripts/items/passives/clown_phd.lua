...
local function replacePillEffects(_, pilleffect, color)
    ...
    if(pillpool and pillpool~=0) then
        ...
        if(PlayerManager.AnyoneHasCollectible(mod.COLLECTIBLE_CLOWN_PHD)) then
            local rng = mod:generateRng()
            return mod:getRandomPillEffect(chosenPlayer:GetCollectibleRNG(mod.COLLECTIBLE_CLOWN_PHD), chosenPlayer, phdVal, {})
        end
    end
end
