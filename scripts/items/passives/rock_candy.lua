...
local function postAddItem(_, _, _, firstTime, slot, vData, player)
    ...
    local mId = mod:getRandomMantle(player:GetCollectibleRNG(mod.COLLECTIBLE_ROCK_CANDY), true)
...
mod:AddCallback(ModCallbacks.MC_POST_ADD_COLLECTIBLE, postAddItem, mod.COLLECTIBLE_ROCK_CANDY)

---@param player EntityPlayer
---@param flag CacheFlag
local function evalCache(_, player, flag)
    if(not player:HasCollectible(mod.COLLECTIBLE_ROCK_CANDY)) then return end
    local mult = player:GetCollectibleNum(mod.COLLECTIBLE_ROCK_CANDY)
