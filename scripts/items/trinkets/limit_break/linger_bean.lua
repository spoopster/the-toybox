...
local function postEffectUpdate(_, effect)
    if(not (effect.SpawnerEntity and effect.SpawnerEntity:ToPlayer() and mod:playerHasLimitBreak(effect.SpawnerEntity:ToPlayer()))) then return end
...
mod:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, postEffectUpdate, EffectVariant.BROWN_CLOUD)