...
local function postFamUpdate(_, fam)
    if(not (fam.Player and mod:playerHasLimitBreak(fam.Player))) then return end
    ...
    if(sp:GetAnimation()=="Spawn" and sp:GetFrame()==0) then
        for _, pickup in ipairs(Isaac.FindByType(5)) do
            if(chests[pickup.Variant]==0 and pickup.FrameCount==0) then
                mod:setEntityData(pickup, "TIMED_CHEST_OPEN", {OPEN_FRAMES, fam.Player or Isaac.GetPlayer()})
            end
        end
    end
end
mod:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, postFamUpdate, FamiliarVariant.KEY_BUM)
mod:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, postFamUpdate, FamiliarVariant.SUPER_BUM)

---@param pickup EntityPickup
local function postChestUpdate(_, pickup)
    if(chests[pickup.Variant]~=0) then return end
    local data = mod:getEntityDataTable(pickup)
...
mod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, postChestUpdate)