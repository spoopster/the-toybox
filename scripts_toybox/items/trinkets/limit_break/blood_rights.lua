...
local function postUseItem(_, item, rng, player, flags, slot, vdata)
    ...
    for _, enemy in ipairs(Isaac.FindInRadius(ToyboxMod.GAME:GetRoom():GetCenterPos(), 1000, EntityPartition.ENEMY)) do
        if(enemy:IsEnemy() and enemy:HasMortalDamage() and rng:RandomFloat()<HEART_CHANCE) then
            local heart = Isaac.Spawn(5,10,2,ToyboxMod.GAME:GetRoom():FindFreePickupSpawnPosition(enemy.Position),Vector.Zero,player):ToPickup()
        end
    end
end
ToyboxMod:AddCallback(ModCallbacks.MC_USE_ITEM, postUseItem, CollectibleType.COLLECTIBLE_BLOOD_RIGHTS)