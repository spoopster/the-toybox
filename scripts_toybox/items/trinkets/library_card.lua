local function isSilverTreasureRoom(idx)
    if(ToyboxMod.GAME:IsGreedMode()) then
        return idx==98
    end
    return false
end
...
local function replaceBackdrop()
    if(isLibraryCardLibrary(ToyboxMod.GAME:GetLevel():GetCurrentRoomIndex())) then
...
local function tryMarkRoomAsPermaLibrary()
    ...
    if(isLibraryCardLibrary(room.SafeGridIndex) or (room.VisitedCount==1 and room.Data and room.Data.Type==RoomType.ROOM_TREASURE)) then
        ...
        ToyboxMod.GAME:GetRoom():SetItemPool(ItemPoolType.POOL_LIBRARY)
    end
end
...
local function changeToLibrary(_, door)
    ...
    local iscurrentlibrary = isLibraryCardLibrary(ToyboxMod.GAME:GetLevel():GetCurrentRoomIndex())
    ...
    if(ToyboxMod.GAME:GetRoom():GetFrameCount()>0 and spawnpoof) then
