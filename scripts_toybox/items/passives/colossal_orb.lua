...
local function evalCache(_, pl, flag)
    if(not pl:HasCollectible(mod.COLLECTIBLE.COLOSSAL_ORB)) then return end
    local mult = pl:GetCollectibleNum(mod.COLLECTIBLE.COLOSSAL_ORB)
                ...
                function(effect)
                    ...
                    function(timer)
                        ...
                        for _, ent in pairs(enemies) do
                            --print("B", timer.FrameCount, mod:getEntityData(ent, "ORB_CONCUSSED"), prevDist, ent.Position:Distance(effect.Position), curDist, "|", ent.Type, ent.Variant)

                            if(not mod:getEntityData(ent, "ORB_CONCUSSED") and ent.Position:Distance(effect.Position)>=prevDist) then
                                mod:setEntityData(ent, "ORB_CONCUSSED", true)
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, shockwaveOnEntry)