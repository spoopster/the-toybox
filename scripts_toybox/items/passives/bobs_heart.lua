...
local EXPL_CHANCE = 1--0.5

---@param pl EntityPlayer
local function cancelExplosionDmg(_, pl, damage, flags, source, count)
    if(not pl:HasCollectible(ToyboxMod.COLLECTIBLE_BOBS_HEART)) then return end
    if(flags & DamageFlag.DAMAGE_EXPLOSION~=0) then return false end
end
ToyboxMod:AddCallback(ModCallbacks.MC_PRE_PLAYER_TAKE_DMG, cancelExplosionDmg, 0)
...
local function applyMarkPenalties(_, player, _, flags, source)
    ...
    --if(player:GetCollectibleRNG(ToyboxMod.COLLECTIBLE_BOBS_HEART):RandomFloat()<EXPL_CHANCE) then
...
ToyboxMod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, applyMarkPenalties, EntityType.ENTITY_PLAYER)