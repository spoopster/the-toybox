...
local function postUpdate()
    if((Ambush.GetCurrentWave()-lastWaveVal)>0 and Ambush.GetCurrentWave()>1) then
        Isaac.RunCallback(ToyboxMod.CUSTOM_CALLBACKS.POST_ROOM_CLEAR, Ambush.GetCurrentWave()-1)
    end
...
ToyboxMod:AddPriorityCallback(ModCallbacks.MC_PRE_SPAWN_CLEAN_AWARD, CallbackPriority.IMPORTANT, postRoomClear)