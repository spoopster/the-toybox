...
function ToyboxMod:registerThrowableActive(item, willDischarge, willRemove)
    ...
    ToyboxMod.THROWABLE_ITEMS[item] = {
...
local function useThrowable(_, item, rng, player, flags, slot)
    ...
    elseif(ToyboxMod.THROWABLE_ITEMS[item]) then
        ...
        pData.THROWABLE_DATA = {
            ...
            FLAGS=flags,
            SLOT=slot,
            DISCHARGE = ToyboxMod.THROWABLE_ITEMS[item].DISCHARGE,
            REMOVE = ToyboxMod.THROWABLE_ITEMS[item].REMOVE,
        }
...
ToyboxMod:AddPriorityCallback(ModCallbacks.MC_USE_ITEM, CallbackPriority.LATE, useThrowable)
    ...
    if(pData.THROWABLE_DATA) then
        if(player:GetItemState()==pData.THROWABLE_DATA.ITEM) then
            ...
            if(joystick:Length()>0.05) then
                ...
                Isaac.RunCallbackWithParam(ToyboxMod.CUSTOM_CALLBACKS.USE_THROWABLE_ACTIVE, pData.THROWABLE_DATA.ITEM, pData.THROWABLE_DATA.ITEM, player, pData.THROWABLE_DATA.RNG, pData.THROWABLE_DATA.FLAGS, pData.THROWABLE_DATA.SLOT, 1)
...
ToyboxMod:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, updateThrowable)
...
ToyboxMod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, cancelThrowableOnNewRoom)