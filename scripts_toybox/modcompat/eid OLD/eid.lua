...
local descs = include("scripts_toybox.modcompat.eid.enums")
            ...
            function(descObj)
                for _, mData in ipairs(desc.DescriptionAppend) do
                    if((mData.Condition and mData.Condition(descObj)) or (not mData.Condition)) then
                        local textToAdd = mData.DescriptionToAdd
                        if(type(textToAdd)=="function") then
                            textToAdd = textToAdd(descObj)
                        end

                        if(mData.AddToTop==true) then
                            descObj.Description = turnStringTableToEIDDesc(textToAdd).."#"..descObj.Description
                        else
                            descObj.Description = descObj.Description.."#"..turnStringTableToEIDDesc(textToAdd)
                        end
                    end
                end
