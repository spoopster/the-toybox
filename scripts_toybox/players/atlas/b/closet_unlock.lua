...
local function replaceCollectibleInCloset(_, pickup)
    ...
    if(roomConfig.Type==1 and roomConfig.Variant==6 and roomConfig.Subtype==11) then
        if(ToyboxMod:isAtlasA(Isaac.GetPlayer()) and Isaac.GetPersistentGameData():Unlocked(ToyboxMod.ACHIEVEMENT.ATLAS_B)==false) then
...
local function postMilcomSecretUpdate(_, slot)
    ...
    if(ToyboxMod:isAtlasA(Isaac.GetPlayer()) and Isaac.GetPersistentGameData():Unlocked(ToyboxMod.ACHIEVEMENT.ATLAS_B)==false) then
        ToyboxMod:unlock(ToyboxMod.ACHIEVEMENT.ATLAS_B)
    end
end
