...
function ToyboxMod:addBloodSoulCounter(player, num)
...
local function bloodSoulInit(_, pickup)
    ...
    ToyboxMod:setEntityData(trail, "BLOOD_SOUL_PARENT", pickup)

    trail:Update()
end
ToyboxMod:AddCallback(ModCallbacks.MC_POST_PICKUP_INIT, bloodSoulInit, ToyboxMod.PICKUP_VARIANT.BLOOD_SOUL)

---@param pickup EntityPickup
local function bloodSoulUpdate(_, pickup)
    if(pickup.SpawnerEntity and pickup.SpawnerEntity:ToPlayer()) then
        pickup.Velocity = ToyboxMod:lerp(pickup.Velocity, (pickup.SpawnerEntity.Position-pickup.Position):Resized(BLOOD_SOUL_SPEED), 0.05)
    else
        pickup.Velocity = pickup.Velocity*0.95
    end
end
ToyboxMod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, bloodSoulUpdate, ToyboxMod.PICKUP_VARIANT.BLOOD_SOUL)
...
local function bloodSoulCollision(_, pickup, coll)
    ...
    ToyboxMod:addBloodSoulCounter(coll:ToPlayer(), 1)
...
ToyboxMod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CallbackPriority.LATE, bloodSoulCollision, ToyboxMod.PICKUP_VARIANT.BLOOD_SOUL)

local function soulTrailRender(_, effect, offset)
    local pickupParent = ToyboxMod:getEntityData(effect, "BLOOD_SOUL_PARENT")
...
local function evalBlood(_, player, flag)
    ...
    if(flag==CacheFlag.CACHE_DAMAGE) then
        ToyboxMod:addBasicDamageUp(player, boostIntensity*BLOOD_DMG_UP)
    elseif(flag==CacheFlag.CACHE_FIREDELAY) then
        ToyboxMod:addBasicTearsUp(player, boostIntensity*BLOOD_TEARS_UP)
    end
end
ToyboxMod:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, evalBlood)
...
ToyboxMod:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, decreaseBloodCounter)