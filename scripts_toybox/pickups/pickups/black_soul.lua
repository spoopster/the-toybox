...
--! PICKUP LOGIC
function ToyboxMod:getBlackSpiritNum(player)
    local numShadows = math.max(0, ToyboxMod:getEntityData(player, "BLACKSOUL_COUNTER") or 0)
    return numShadows--math.floor((numShadows)^(1/2))
end

---@param player EntityPlayer
function ToyboxMod:addBlackSoulCounter(player, num)
    local data = ToyboxMod:getEntityDataTable(player)
    local oldNum = ToyboxMod:getBlackSpiritNum(player)
...
local function blackSoulInit(_, pickup)
    ...
    ToyboxMod:setEntityData(trail, "BLACK_SOUL_PARENT", pickup)

    trail:Update()
end
ToyboxMod:AddCallback(ModCallbacks.MC_POST_PICKUP_INIT, blackSoulInit, ToyboxMod.PICKUP_VARIANT.BLACK_SOUL)

---@param pickup EntityPickup
local function blackSoulUpdate(_, pickup)
    if(pickup.SpawnerEntity and pickup.SpawnerEntity:ToPlayer()) then
        pickup.Velocity = ToyboxMod:lerp(pickup.Velocity, (pickup.SpawnerEntity.Position-pickup.Position):Resized(BLACK_SOUL_SPEED), 0.05)
    else
        pickup.Velocity = pickup.Velocity*0.95
    end
end
ToyboxMod:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, blackSoulUpdate, ToyboxMod.PICKUP_VARIANT.BLACK_SOUL)
...
local function blackSoulCollision(_, pickup, coll)
    ...
    ToyboxMod:addBlackSoulCounter(coll:ToPlayer(), 1)
...
ToyboxMod:AddPriorityCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, CallbackPriority.LATE, blackSoulCollision, ToyboxMod.PICKUP_VARIANT.BLACK_SOUL)

local function soulTrailRender(_, effect, offset)
    local pickupParent = ToyboxMod:getEntityData(effect, "BLACK_SOUL_PARENT")
...
ToyboxMod:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, soulTrailRender, EffectVariant.SPRITE_TRAIL)

--! FAMILIAR LOGIC
---@param player EntityPlayer
local function evalSpirits(_, player, flag)
    local data = ToyboxMod:getEntityDataTable(player)
    player:CheckFamiliar(ToyboxMod.FAMILIAR_VARIANT.EVIL_SHADOW, ToyboxMod:getBlackSpiritNum(player), player:GetCardRNG(ToyboxMod.CONSUMABLE.MANTLE_DARK), nil)
end
ToyboxMod:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, evalSpirits, CacheFlag.CACHE_FAMILIARS)
...
local function blackSpiritInit(_, fam)
    ...
    local otherSpirits = Isaac.FindByType(EntityType.ENTITY_FAMILIAR, ToyboxMod.FAMILIAR_VARIANT.EVIL_SHADOW)
    if(#otherSpirits>0) then
        ...
        for _, ent in ipairs(otherSpirits) do
            ...
            if(ent.State==0 and GetPtrHash(ent.Player)==playerHash) then
                local eData = ToyboxMod:getEntityDataTable(ent)
...
ToyboxMod:AddCallback(ModCallbacks.MC_FAMILIAR_INIT, blackSpiritInit, ToyboxMod.FAMILIAR_VARIANT.EVIL_SHADOW)
...
local function blackSpiritUpdate(_, fam)
    ...
    if(fam.State==0) then -- IDLE/FOLLOWING
        ...
        fam.Velocity = ToyboxMod:lerp(fam.Velocity, (data.BLACKSPIRIT_DELAYEDPOS[2]-data.BLACKSPIRIT_DELAYEDPOS[1]), 0.75)
        ...
        fam.CollisionDamage = SPIRIT_DMG+(ToyboxMod:getEntityData(fam.Player, "BLACKSOUL_COUNTER") or 0)*SPIRIT_DMGPERSOUL
    ...
    elseif(fam.State==1) then
        if(fam.Target==nil or (fam.Target and (fam.Target:IsDead() or not fam.Target:Exists()))) then
            ...
            for _, ent in ipairs(possibleTargets) do
                if(ToyboxMod:isValidEnemy(ent)) then table.insert(validTargets, ent:ToNPC()) end
            end
        ...
        local newVel = dir:Resized(ToyboxMod:lerp(SPIRIT_CRAZYSPEED, SPIRIT_CRAZYFASTERSPEED, math.max(fl, 0))*(rng:RandomFloat()*0.1+0.9))
        --if(newVel:Length()>SPIRIT_CRAZYSPEED) then newVel = newVel:Resized(SPIRIT_CRAZYSPEED) end
        fam.Velocity = ToyboxMod:lerp(fam.Velocity, newVel, 0.05)
        ...
        fam.CollisionDamage = SPIRIT_CRAZYDMG+(ToyboxMod:getEntityData(fam.Player, "BLACKSOUL_COUNTER") or 0)*SPIRIT_DMGPERSOUL
        fam.Color = Color(1,1,1,alpha)
    end
end
ToyboxMod:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, blackSpiritUpdate, ToyboxMod.FAMILIAR_VARIANT.EVIL_SHADOW)
...
local function spiritNewRoom(_)
    ...
    for i=0, Game():GetNumPlayers()-1 do
        ...
        if(data.BLACKSOUL_COUNTER and data.BLACKSOUL_COUNTER>0 and data.BLACKSOUL_GOTHIT and data.BLACKSOUL_GOTHIT>0) then
            ...
            pl:CheckFamiliar(ToyboxMod.FAMILIAR_VARIANT.EVIL_SHADOW, 0, ToyboxMod:generateRng())
...
ToyboxMod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, spiritNewRoom)
...
local function soulDamage(_, player, _, flags, source)
    ...
    ToyboxMod:setEntityData(player, "BLACKSOUL_GOTHIT", ToyboxMod:getEntityData(player, "BLACKSOUL_COUNTER"))
...
ToyboxMod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, soulDamage, EntityType.ENTITY_PLAYER)