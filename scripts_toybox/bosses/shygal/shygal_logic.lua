...
local function pickAttack(npc)
    ...
    if(data.SHYGAL_PREV_ATTACK~="REVIVE" and npc.Variant==ToyboxMod.BOSS_SHYGAL and data.SHYGAL_CLONES and data.SHYGAL_NUMCLONES) then
...
local function pickMovementPos(npc)
    ...
    local failsafe=100
    repeat
        newPos = ToyboxMod:getRandomFreePos()
        failsafe = failsafe-1
...
local function calcAngerValue(npc, start, final, anger) --* currently just returns start because anger mechanic is unnecessary
    return start
    --return math.floor(ToyboxMod:lerp(start, final, math.min(1,ToyboxMod:getEntityData(npc, "SHYGAL_AGGRESSION")/anger)))
end
---@param npc EntityNPC
local function updateMaskState(npc, isMasked, doEffects)
    ...
    if(isMasked) then
        ...
        if(doEffects and npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE) then
            data.SHYGAL_HASMASK = true
            npc:Kill()
        end
    end
...
local function movementLogic(npc, speed, targetpos, movementLerp, failMoveFriction, minTargetDist, walkAnimSpeedmult)
    ...
    local forceMove
    if(ToyboxMod:isScared(npc)) then
    ...
    if(npc.Position:DistanceSquared(posToMove)>(minTargetDist or 1)*(minTargetDist or 1) and (npc.Pathfinder:HasPathToPos(posToMove,false) or forceMove)) then
        ...
        vel = ToyboxMod:lerp(npc.Velocity, newvel or Vector.Zero, (movementLerp or 0.05))
        sprite.PlaybackSpeed = walkAnimSpeedmult or 1--ToyboxMod:getWalkAnimPlaybackSpeed(npc, speed/(walkAnimSpeedmult or 1), true)
    end
...
local function shygalsInit(_, npc)
    ...
    if(npc.Variant==ToyboxMod.BOSS_SHYGAL or npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE) then
        --print("ok")
    ...
    if(npc.Variant==ToyboxMod.BOSS_SHYGAL) then
        ...
        for i=1, numClones do
            local clone = Isaac.Spawn(ToyboxMod.NPC_BOSS, ToyboxMod.NPC_SHYGAL_CLONE, SHYGAL_W_MASK_SUBTYPE, npc.Position, Vector.Zero, npc):ToNPC()
            ToyboxMod:setEntityData(clone, "MOVEMENT_POS", p[i+1])
    ...
    if(npc.Variant==ToyboxMod.NPC_SHYGAL_MASK) then
        npc.GridCollisionClass = EntityGridCollisionClass.GRIDCOLL_NONE
        npc:GetSprite():Play("Idle", true)
        npc.CanShutDoors = false
    end
end
ToyboxMod:AddCallback(ModCallbacks.MC_POST_NPC_INIT, shygalsInit, ToyboxMod.NPC_BOSS)
...
local function shygalsLogic(_, npc)
    ...
    if(npc.Variant==ToyboxMod.BOSS_SHYGAL or npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE) then
        ...
        if(data.SHYGAL_STATE=="IDLE") then
            ...
            if(npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE) then data.SHYGAL_AGGRESSION = data.SHYGAL_AGGRESSION+1 end
        ...
        elseif(data.SHYGAL_STATE=="SPIT") then
            ...
            if(sprite:IsEventTriggered("Attack")) then
                local p = npc.Target or ToyboxMod:closestPlayer(npc.Position)
    ...
    elseif(npc.Variant==ToyboxMod.NPC_SHYGAL_MASK) then
        if(npc.FrameCount%60==0 or npc.Target==nil or (npc.Target and npc.Target:IsDead() or not npc.Target:Exists())) then
            npc.Target = npc:GetPlayerTarget()
        end

        --[[
        if(npc.SpawnerEntity and npc.SpawnerEntity.Type==ToyboxMod.NPC_BOSS and npc.SpawnerEntity.Variant==ToyboxMod.BOSS_SHYGAL and npc.SpawnerEntity.SubType==TRUE_SUBTYPE) then
            local spawnerTb = ToyboxMod:getEntityDataTable(npc.SpawnerEntity)
            local idx = 1
            ...
            npc.Velocity = ToyboxMod:lerp(npc.Velocity, newVel, 0.35)
        else
        --]]
            npc.Velocity = Vector.Zero
            if(ToyboxMod:isRoomClear()) then npc:Kill() end
        --end
...
ToyboxMod:AddCallback(ModCallbacks.MC_PRE_NPC_UPDATE, shygalsLogic, ToyboxMod.NPC_BOSS)
...
local function shygalsDead(_, npc)
    ...
    local mask
    if(npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE and npc.SubType & 1 == 1) then
        mask = Isaac.Spawn(ToyboxMod.NPC_BOSS, ToyboxMod.NPC_SHYGAL_MASK, 0, npc.Position, Vector.Zero, npc.SpawnerEntity):ToNPC()
    end
    if(npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE and npc.SpawnerEntity and npc.SpawnerEntity.Type==ToyboxMod.NPC_BOSS and npc.SpawnerEntity.Variant==ToyboxMod.BOSS_SHYGAL) then
        local spawnerTb = ToyboxMod:getEntityDataTable(npc.SpawnerEntity)
...
ToyboxMod:AddCallback(ModCallbacks.MC_POST_ENTITY_KILL, shygalsDead, ToyboxMod.NPC_BOSS)

local function shygalsCollision(_, npc, coll, low)
    if(not (npc.Variant==ToyboxMod.BOSS_SHYGAL or npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE or npc.Variant==ToyboxMod.NPC_SHYGAL_MASK)) then return end

    if(coll and coll.Type==ToyboxMod.NPC_BOSS and (coll.Variant==ToyboxMod.BOSS_SHYGAL or coll.Variant==ToyboxMod.NPC_SHYGAL_CLONE or coll.Variant==ToyboxMod.NPC_SHYGAL_MASK)) then return true end
end
ToyboxMod:AddCallback(ModCallbacks.MC_PRE_NPC_COLLISION, shygalsCollision, ToyboxMod.NPC_BOSS)

---@param npc EntityNPC
local function shygalsTakeDmg(_, npc, amount, flags, source, frames)
    if(not (npc.Variant==ToyboxMod.BOSS_SHYGAL or npc.Variant==ToyboxMod.NPC_SHYGAL_CLONE or npc.Variant==ToyboxMod.NPC_SHYGAL_MASK)) then return end
    if(npc.Variant==ToyboxMod.NPC_SHYGAL_MASK) then return false end
...
ToyboxMod:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, shygalsTakeDmg, ToyboxMod.NPC_BOSS)